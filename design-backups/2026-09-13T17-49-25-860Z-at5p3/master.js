...
const Master = {
  ...
  addItem(b, ctx) {
    ...
    const d = store.load();
    if (!d.categories.length) throw Object.assign(new Error('ترتيب إلزامي: أدخل تصنيفات المواد أولاً قبل المواد'), { code: 400 });
    ...
    if (!d.categories.some(c => c.id === Number(b.category_id))) throw Object.assign(new Error('التصنيف غير موجود — اختره من تبويب التصنيفات'), { code: 400 });
    ...
    const row = {
      ...
      category_id: Number(b.category_id), unit: b.unit,
  ...
  updateItem(id, b, ctx) {
    ...
    if (b.last_price !== undefined) r.last_price = toNum(b.last_price, 'آخر سعر شراء');
  ...
  // ---- أسماء التركيبات (مرجعي) ----
  addFormula(b, ctx) {
    if (!hasText(b.name)) throw Object.assign(new Error('اسم التركيبة مطلوب'), { code: 400 });
    const d = store.load();
    const row = { id: d.seq.formula++, name: b.name.trim(), description: (b.description || '').trim(), status: b.status === 'متوقفة' ? 'متوقفة' : 'نشطة', created_at: now(), updated_at: now() };
    d.formulas_ref.push(row);
    audit(d, 'add', 'formula_ref', row.id, ctx.user, ctx.role);
    store.save(d);
    return { data: row };
  },
  updateFormula(id, b, ctx) {
    const d = store.load();
    ...
    if (b.status !== undefined) r.status = b.status === 'متوقفة' ? 'متوقفة' : 'نشطة';
    r.updated_at = now();
    audit(d, 'update', 'formula_ref', id, ctx.user, ctx.role);
    store.save(d);
    return { data: r };
  },
