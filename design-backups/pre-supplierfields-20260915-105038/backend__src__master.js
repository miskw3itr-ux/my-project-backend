...
function fiscalNum(v, label) {
  const s = String(v === undefined || v === null ? '' : v).trim().replace(/\s+/g, ' ');
  if (s.length > 40) throw Object.assign(new Error(label + ': 40 حرفاً على الأكثر'), { code: 400 });
  return s;
}
...
const Master = {
  ...
  _addParty(col, logEntity, b, ctx) {
    ...
    if (col === 'customers') {
      // البطاقة التجارية للزبون: صنف + نوع سعر (افتراضي تجزئة) + أرقام جبائية اختيارية
