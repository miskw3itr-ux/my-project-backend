...
const CAN_DEPOSIT = ['admin', 'accountant'];
const CAN_CONFIRM = ['admin', 'storekeeper'];
...
const Proc = {
  ...
  addOrder(b, ctx) {
    ...
    const row = {
      id: d.seq.order++, num: 'BC-' + year + '-' + String(d.seq.order_num++).padStart(3, '0'),
  ...
  decideOrder(id, b, ctx) {
    if (ctx.role !== 'admin') err('الموافقة والرفض للمدير فقط', 403);
    const d = db.load();
    const o = d.orders.find(x => x.id === Number(id));
    if (!o) err('الطلبية غير موجودة', 404);
  ...
  addDeposit(b, ctx) {
    if (!CAN_DEPOSIT.includes(ctx.role)) err('العربون: المحاسب أو المدير فقط', 403);
    const d = db.load();
  ...
  addConfirmation(b, ctx) {
    if (!CAN_CONFIRM.includes(ctx.role)) err('التأكيد: أمين المخزن أو المدير فقط', 403);
    const md = mdata();
    const d = db.load();
    const o = d.orders.find(x => x.id === Number(b.order_id));
    if (!o) err('الطلبية غير موجودة', 404);
    ...
    for (const l of lines) {
      if (l.received_qty > 0) {
        Stock.receiveLot({ item_id: l.item_id, item_name: l.item_name, unit: l.unit, qty: l.received_qty, cost: l.price, warehouse_id: wh.id, warehouse_name: wh.name, expiry, qc: qc.result, order_num: o.num, supplier_name: o.supplier_name, date: arrival });
