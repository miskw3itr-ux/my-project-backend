...
(function () {
  ...
  function ensureHead() {
    ...
    // شعار المصنع الرسمي (logo.png) يغلب أي أيقونة قديمة في كل الصفحات
    try {
      document.querySelectorAll('link[rel="icon"]').forEach(l => { l.type = 'image/png'; l.href = base + 'assets/logo/logo.png'; });
    } catch {}
    if (!document.querySelector('link[rel="icon"]')) {
      const f = document.createElement('link');
      f.rel = 'icon'; f.type = 'image/png'; f.href = base + 'assets/logo/logo.png';
      head.appendChild(f);
    }
    ...
    try {
      ...
      try {
        document.querySelectorAll('link[rel="apple-touch-icon"]').forEach(l => { l.href = base + 'assets/logo/logo.png'; });
      } catch {}
      if (!document.querySelector('link[rel="apple-touch-icon"]')) {
        const at = document.createElement('link');
        at.rel = 'apple-touch-icon'; at.href = base + 'assets/logo/logo.png';
        head.appendChild(at);
      }
  ...
  function upgradeSidebar() {
    ...
    if (h2 && !sb.querySelector('.brand')) {
      ...
      brand.innerHTML = '<img src="' + base + 'assets/logo/logo.png" alt="دار العلف">'
        + '<div><div class="brand-name">دار العلف</div><div class="brand-sub">إدارة مصنع الأعلاف</div></div>';
      h2.replaceWith(brand);
    }
    // إصلاح مسار الشعار حسب العمق (الشعار الرسمي PNG)
    const logo = sb.querySelector('.brand img');
    if (logo && !/logo\.png(\?|$)/.test(logo.src)) logo.src = base + 'assets/logo/logo.png';
