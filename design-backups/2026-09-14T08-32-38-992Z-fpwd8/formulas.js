...
const Formulas = {
  costing(overheadPct) {
    ...
    const data = d.formulas_ref.map(f => {
      ...
      const total = r2(materials + ohAmt);
      const totQty = r2(lines.reduce((s, l) => s + Number(l.qty || 0), 0));
      return { id: f.id, num: f.num || '—', name: f.name, date: f.date || '', status: f.status || 'نشطة', note: f.note || '', parent_num: f.parent_num || null, items_count: lines.length, totQty, materials, overhead_pct: oh, overhead: ohAmt, total, perQ: totQty > 0 ? r2(total / totQty) : 0, lines };
    }).sort((a, b) => (String(a.num) < String(b.num) ? 1 : -1));
    const act = data.filter(f => f.status === 'نشطة');
    const byQ = act.slice().sort((a, b) => a.perQ - b.perQ);
    let lastChg = null;
    ...
    return {
      ...
      kpi: {
        active: act.length,
        cheapest: byQ[0] ? { num: byQ[0].num, name: byQ[0].name, perQ: byQ[0].perQ } : null,
        expensive: byQ.length ? { num: byQ[byQ.length - 1].num, name: byQ[byQ.length - 1].name, perQ: byQ[byQ.length - 1].perQ } : null,
        avgQ: act.length ? r2(act.reduce((s, f) => s + f.perQ, 0) / act.length) : 0,
        lastChange: lastChg ? { ...lastChg, at: String(lastChg.at).slice(0, 10), affected } : null
      },
      overhead: oh, at: new Date().toISOString().slice(0, 10)
    };
  }
};
