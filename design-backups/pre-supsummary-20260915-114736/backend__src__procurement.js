...
const Proc = {
  ...
  supplierActivity(supplier_id, from, to) {
    ...
    for (const x of d.deposits || []) {
      ...
      lines.push({ date: x.date, kind: x.method === 'مؤخر' ? 'وعد مؤخر' : 'دفعة', ref: x.order_num || '', detail: x.method + (x.note ? ' — ' + x.note : ''), total: r2(Number(x.amount || 0)), effect: -cash, _id: 'd' + x.id });
    }
