...
const Analytics = {
  ...
  cost(q, ctx) {
    ...
    const rows = (d.lots || []).filter(l => finType(l.warehouse_id, md) === 'منتج نهائي'
      && Number(l.cost_per_unit || 0) > 0 && String(l.created_at || '').slice(0, 10) >= from && String(l.created_at || '').slice(0, 10) <= to);
    const avg = (rs) => {
      let v = 0, tn = 0;
    ...
    const data = fill(list, b => {
      const r = rows.filter(l => { const ds = String(l.created_at || '').slice(0, 10); return ds >= b.from && ds <= b.to; });
