...
const Master = {
  ...
  _ensureProduct(d, frow, ctx) {
    ...
    let code = 'P-' + frow.num;
    if (d.items.some(x => x.code.trim() === code)) code = code + '-' + frow.id;
    const cat = resolveCategory(d, 'منتج نهائي', 'منتجات التركيبات', ctx);
    const row = {
      id: d.seq.item++, code, name: nm,
  ...
  makeProduct(id, ctx) {
    if (!CAN_MATERIAL.includes(ctx.role)) throw Object.assign(new Error('صلاحية البطاقات: مدير المخزون أو المدير العام فقط'), { code: 403 });
    const d = store.load();
